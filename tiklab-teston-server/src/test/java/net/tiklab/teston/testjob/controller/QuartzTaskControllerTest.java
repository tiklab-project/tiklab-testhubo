...
public class QuartzTaskControllerTest {

    private static Logger logger = LoggerFactory.getLogger(QuartzTaskControllerTest.class);
    ...
    public void test01ForSaveQuartzTask() {
        ...
        MultiValueMap<String, String> multiValueMap = MapUtils.toMultiMap(quartzTask);

        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/quartzTask/createQuartzTask")
    ...
    public void test02ForUpdateQuartzTask(){
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/quartzTask/updateQuartzTask")
                                .params(multiValueMap)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                    )
    ...
    public void test03ForFindQuartzTask() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/quartzTask/findQuartzTask")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
    ...
    public void test04ForFindAllQuartzTask() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/quartzTask/findAllQuartzTask")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteQuartzTask(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/quartzTask/deleteQuartzTask")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
