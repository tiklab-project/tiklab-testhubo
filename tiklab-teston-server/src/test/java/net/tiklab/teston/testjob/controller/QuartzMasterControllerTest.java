...
public class QuartzMasterControllerTest {

    private static Logger logger = LoggerFactory.getLogger(QuartzMasterControllerTest.class);
    ...
    public void test01ForSaveQuartzMaster() {
        ...
        MultiValueMap<String, String> multiValueMap = MapUtils.toMultiMap(quartzMaster);

        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/quartzMaster/createQuartzMaster")
    ...
    public void test02ForUpdateQuartzMaster(){
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/quartzMaster/updateQuartzMaster")
                                .params(multiValueMap)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                    )
    ...
    public void test03ForFindQuartzMaster() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/quartzMaster/findQuartzMaster")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
    ...
    public void test04ForFindAllQuartzMaster() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/quartzMaster/findAllQuartzMaster")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteQuartzMaster(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/quartzMaster/deleteQuartzMaster")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
