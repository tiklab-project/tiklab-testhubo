package net.tiklab.teston.integration.agentconfig.controller;
...
public class AgentConfigControllerTest {
    ...
    private static Logger logger = LoggerFactory.getLogger(AgentConfigControllerTest.class);
    ...
    public void test01ForSaveAgentConfig() {
        ...
        Map paramMap  = JSONObject.parseObject(JSONObject.toJSONString(agentConfig));
        MultiValueMap<String, String> multiValueMap = new LinkedMultiValueMap<>();
        multiValueMap.setAll(paramMap);
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/agentConfig/createAgentConfig")
    ...
    public void test02ForUpdateAgentConfig(){
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/agentConfig/updateAgentConfig")
                                .params(multiValueMap)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                    )
    ...
    public void test03ForFindAgentConfig() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/agentConfig/findAgentConfig")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
    ...
    public void test04ForFindAllAgentConfig() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/agentConfig/findAllAgentConfig")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteAgentConfig(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/agentConfig/deleteAgentConfig")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
