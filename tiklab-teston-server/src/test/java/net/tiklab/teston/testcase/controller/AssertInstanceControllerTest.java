...
public class AssertInstanceControllerTest {

    private static Logger logger = LoggerFactory.getLogger(AssertInstanceControllerTest.class);
    ...
    public void test03ForFindAssertInstance() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/assertInstance/findAssertInstance")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
    ...
    public void test04ForFindAllAssertInstance() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/assertInstance/findAllAssertInstance")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteAssertInstance(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/assertInstance/deleteAssertInstance")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
