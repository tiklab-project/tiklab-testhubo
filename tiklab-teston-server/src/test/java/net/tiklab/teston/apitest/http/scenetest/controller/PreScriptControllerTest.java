...
import net.tiklab.teston.test.api.http.unit.cases.model.PreScript;
...
public class PreScriptControllerTest {

    private static Logger logger = LoggerFactory.getLogger(PreScriptControllerTest.class);
    ...
    public void test01ForSavePreScript() {
        ...
        MultiValueMap<String, String> multiValueMap = MapUtils.toMultiMap(preScript);

        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/preScript/createPreScript")
    ...
    public void test02ForUpdatePreScript(){
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/preScript/updatePreScript")
                                .params(multiValueMap)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                    )
    ...
    public void test04ForFindAllPreScript() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/preScript/findAllPreScript")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeletePreScript(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/preScript/deletePreScript")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
