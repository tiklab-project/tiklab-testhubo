...
import net.tiklab.teston.test.api.http.unit.cases.model.FormUrlencoded;
...
public class FormUrlencodedControllerTest {

    private static Logger logger = LoggerFactory.getLogger(FormUrlencodedControllerTest.class);
    ...
    public void test01ForSaveFormUrlencoded() {
        ...
        Map paramMap  = JSONObject.parseObject(JSONObject.toJSONString(formUrlencoded));
        MultiValueMap<String, String> multiValueMap = new LinkedMultiValueMap<>();
        multiValueMap.setAll(paramMap);
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/formUrlencoded/createFormUrlencoded")
    ...
    public void test02ForUpdateFormUrlencoded(){
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/formUrlencoded/updateFormUrlencoded")
                                .params(multiValueMap)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                    )
    ...
    public void test03ForFindFormUrlencoded() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/formUrlencoded/findFormUrlencoded")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
    ...
    public void test04ForFindAllFormUrlencoded() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/formUrlencoded/findAllFormUrlencoded")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteFormUrlencoded(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/formUrlencoded/deleteFormUrlencoded")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
