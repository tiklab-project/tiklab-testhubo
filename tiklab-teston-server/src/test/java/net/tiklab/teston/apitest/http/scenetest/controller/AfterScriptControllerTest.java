...
public class AfterScriptControllerTest {

    private static Logger logger = LoggerFactory.getLogger(AfterScriptControllerTest.class);
    ...
    public void test03ForFindAfterScript() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/afterScript/findAfterScript")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
    ...
    public void test04ForFindAllAfterScript() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/afterScript/findAllAfterScript")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteAfterScript(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/afterScript/deleteAfterScript")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
