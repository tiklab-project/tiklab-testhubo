...
import net.tiklab.teston.test.api.http.unit.cases.model.JsonParam;
...
public class JsonParamControllerTest {

    private static Logger logger = LoggerFactory.getLogger(JsonParamControllerTest.class);
    ...
    public void test01ForSaveJsonParam() {
        ...
        MultiValueMap<String, String> multiValueMap = MapUtils.toMultiMap(jsonParam);

        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/jsonParam/createJsonParam")
    ...
    public void test02ForUpdateJsonParam(){
        ...
        try {
            MvcResult mvcResult = mockMvc.perform(
                                post("/jsonParam/updateJsonParam")
                                .params(multiValueMap)
                                .accept(MediaType.APPLICATION_JSON)
                                .contentType(MediaType.APPLICATION_JSON)
                    )
    ...
    public void test04ForFindAllJsonParam() {
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/jsonParam/findAllJsonParam")
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
                    .andExpect(status().isOk())
    ...
    public void test05ForDeleteJsonParam(){
        try {
            MvcResult mvcResult = mockMvc.perform(
                    post("/jsonParam/deleteJsonParam")
                            .param("id",id)
                            .accept(MediaType.APPLICATION_JSON)
                            .contentType(MediaType.APPLICATION_JSON)
            )
