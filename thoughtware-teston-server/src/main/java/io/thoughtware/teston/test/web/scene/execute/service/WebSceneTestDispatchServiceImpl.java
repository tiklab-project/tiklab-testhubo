...
public class WebSceneTestDispatchServiceImpl implements WebSceneTestDispatchService {
    ...
    public void execute(WebSceneTestRequest webSceneTestRequest) {
        ...
        String webSceneInstanceId = createInitWebInstance(webSceneId);
    ...
    public WebSceneTestResponse result(WebSceneTestRequest webSceneTestRequest) {
        ...
        //status为0执行结束，存入历史
        updateWebInstance(webSceneTestResponse,webSceneTestRequest.getWebSceneId());
    ...
    public WebSceneTestResponse getResult(WebSceneTestRequest webSceneTestRequest){
        ...
        try{
            ...
            WebSceneInstance webSceneInstance = getInitWebSceneInstance(webSceneTestRequest.getWebSceneId());
            webSceneInstance.setStatus(MagicValue.TEST_STATUS_FAIL);
            webSceneTestResponse.setWebSceneInstance(webSceneInstance);
            throw new ApplicationException(e);
        }
     ...
     */
    @Override
    public String createInitWebInstance(String belongId){
        WebSceneInstance webSceneInstance = getInitWebSceneInstance(belongId);
        String webSceneInstanceId = webSceneInstanceService.createWebSceneInstance(webSceneInstance);

        WebSceneCase webSceneCase = webSceneCaseService.findWebSceneCase(belongId);
        // 创建公共实例
        Instance instance = new Instance();
        instance.setId(webSceneInstanceId);
        instance.setBelongId(belongId);
        ...
        //获取当前执行次数
        int executeNum = instanceService.getRecentExecuteNum(belongId);
     ...
     */
    @Override
    public void updateWebInstance(WebSceneTestResponse webSceneTestResponse, String webSceneId){
        ...
        if (webSceneTestResponse.getWebSceneInstance()!=null&&instanceId!=null) {
            ...
            instanceMap.put("totalDuration",webSceneInstance.getTotalDuration().toString());
            instance.setContent(instanceMap.toString());
            instance.setStatus(webSceneInstance.getStatus());
    ...
    private void updateStatus(String instanceId,String status){
        WebSceneInstance webSceneInstance = webSceneInstanceService.findWebSceneInstance(instanceId);
        webSceneInstance.setStatus(status);
        webSceneInstanceService.updateWebSceneInstance(webSceneInstance);
    }
